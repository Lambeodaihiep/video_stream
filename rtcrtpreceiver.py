...
# new function for thread
def transfer_rtp_packet(
    loop: asyncio.AbstractEventLoop, input_q: queue.Queue, output_q: asyncio.Queue
) -> None:
    ...
    while True:
        ...
        if task is None:
            ...
            # pass the decoded frame to the track
            asyncio.run_coroutine_threadsafe(output_q.put(task), loop)
...
class RemoteStreamTrack(MediaStreamTrack):
    def __init__(self, kind: str, id: Optional[str] = None) -> None:
        ...
        self.rtp_packet_queue: asyncio.Queue = asyncio.Queue()
    ...
    async def recv_rtp_packet(self):
        """
        Receive all the rtp packets
        """
        if self.readyState != "live":
            raise MediaStreamError
        
        rtp_packet = await self.rtp_packet_queue.get()
        if rtp_packet is None:
            self.stop()
            raise MediaStreamError
        # print(len(rtp_packet.payload))
        return rtp_packet
...
class RTCRtpReceiver:
    ...
    def __init__(self, kind: str, transport: RTCDtlsTransport) -> None:
        ...
        self.__rtp_packet_queue: queue.Queue = queue.Queue()                # new line
        self.__decoder_thread: Optional[threading.Thread] = None
        self.__transfer_rtp_packet_thread: Optional[threading.Thread] = None            # new line
        self.__kind = kind
        if kind == "audio":
    ...
    async def receive(self, parameters: RTCRtpReceiveParameters) -> None:
        ...
        if not self.__started:
            ...
            self.__transfer_rtp_packet_thread = threading.Thread(
                target=transfer_rtp_packet,
                name=self.__kind + "-transfer",
                args=(
                    asyncio.get_event_loop(),
                    self.__rtp_packet_queue,
                    self._track.rtp_packet_queue,
                ),
            )
            self.__transfer_rtp_packet_thread.start()
    ...
    async def _handle_rtp_packet(self, packet: RtpPacket, arrival_time_ms: int) -> None:
        ...
        # send the raw RTP packet to the queue
        if self.__transfer_rtp_packet_thread:
            self.__rtp_packet_queue.put(packet)             # new line
    ...
    def __stop_decoder(self) -> None:
        ...
        # new condition
        if self.__transfer_rtp_packet_thread:
            self.__rtp_packet_queue.put(None)
            self.__transfer_rtp_packet_thread.join()
            self.__transfer_rtp_packet_thread = None
